...
public class DolphinLogoSkin extends SkinBase<DolphinLogo, DolphinLogoBehavior> {
    ...
    public Group getDolphin() {
        return dolphin;
    }

    private Group dolphin;
	...
	@Override protected double computeMinWidth(final double MIN_WIDTH) {
	    return super.computeMinWidth(Math.max(401, MIN_WIDTH - getInsets().getLeft() - getInsets().getRight()));
	}
	...
	@Override protected double computeMaxWidth(final double MAX_WIDTH) {
	    return super.computeMaxWidth(Math.max(401, MAX_WIDTH - getInsets().getLeft() - getInsets().getRight()));
	}
    ...
    public final void drawDolphin() {
        ...
        dolphin.getChildren().clear();

        final Shape IBOUNDS = new Rectangle(0, 0, WIDTH, HEIGHT);
        IBOUNDS.setOpacity(0.0);
        dolphin.getChildren().add(IBOUNDS);

        final Path MAINBODY = new Path();
        ...
        final Path LATERALLINE = new Path();
        ...
        dolphin.getChildren().addAll(MAINBODY,
                                     RIP9,
                                     RIP8,
                                     RIP7,
                                     RIP6,
    ...
    private void continueDolphin1(Group dolphin, final double WIDTH, final double HEIGHT) {
        ...
        final Path TAILFINSTART = new Path();
        ...
        dolphin.getChildren().addAll(BACKFIN2,
                                     BACKFIN1,
                                     TAILFINSTART,
                                     TAILFIN1,
                                     TAILFIN2);
